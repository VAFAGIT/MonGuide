...
<div class="h-screen">
  <div class="flex justify-center items-center h-full ">
    ...
    <div class="mb-10 md:mb-16">
      <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">Add travel</h2>
    </div>
    <form @submit.prevent="Add" class="max-w-screen-md grid sm:grid-cols-2 gap-4 mx-auto">
      <div>
          <button></button>
        <label for="first-name" class="inline-block text-gray-800 text-sm sm:text-base mb-2">Date départ*</label>
        <input v-model="travel.date_D" name="first-name" type="date" required class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
      </div>
      ...
      <div>
        ...
        <input v-model="travel.seats" type="number"  name="first-name" required class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
      </div>
...
export default {
    ...
    methods: {
        Add(){
            ...
            console.log(data)
                fetch('http://localhost/api/Voyages/addVoyage/', {
                method: 'POST',
                headers: {
