...
<div class="h-screen">
  <div class="flex justify-center items-center h-full ">
    ...
    <div class="mb-10 md:mb-16">
      <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">Update activitie</h2>
    </div>
    <form @submit.prevent="update" class="max-w-screen-md grid sm:grid-cols-2 gap-4 mx-auto">
      <div>
        ...
        <input v-model="activities_info.id" name="first-name" type='hidden' required class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
...
export default {
    ...
    methods: {
       ...
       const newLocal = `http://localhost/api/Activites/getOneActiv/${id}` 
       ...
       .then((data) =>{
         this.activities_info = data
                  ...
                  id_travel: this.activities_info.id,
                  
                  }
  
                fetch(`http://localhost/api/Activites/update/${this.$route.params.id}`, {
                method: 'POST',
                headers: {
