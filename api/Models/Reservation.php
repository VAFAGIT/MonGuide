...
class Reservation extends Model
{
    ...
    {
        $query = "SELECT voyage.* , reservation.* FROM reservation JOIN voyage ON reservation.id_travel = voyage.id AND voyage.id_users= :id_users ";
        $stmt = $this->_connexion->prepare($query);
        $stmt->bindParam(':id_users', $data->id_users);
    ...
    {
        $query = "SELECT voyage.* , reservation.*  FROM  reservation JOIN voyage ON reservation.id_users =:id_users AND reservation.id_travel = voyage.id ";
        $stmt = $this->_connexion->prepare($query);
        $stmt->bindParam(':id_users', $data->id_users);
        if ($stmt->execute()) {
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
